...
Optional: set `ALLOWED_DEV_ORIGINS` to a comma-separated list of hosts allowed to request
`/_next` dev assets when `pnpm dev` runs behind a proxy or tunnel, for example
`ALLOWED_DEV_ORIGINS=*.example.dev`.
